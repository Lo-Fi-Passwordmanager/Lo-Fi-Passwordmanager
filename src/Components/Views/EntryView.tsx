...
}> = ({item, copyAndClearClipboard}) => {

        if (item.isEntry()) {
                const entry = item as Entry;
                return (
                    <div className="entryViewEntry">
                            <span>Titel:</span>        <span>{entry.title}</span> <button onClick={() => copyAndClearClipboard(entry.title)}>🔗</button>

                            <span>Benutzername:</span> <span>{entry.username}</span> <button onClick={() => copyAndClearClipboard(entry.username)}>🔗</button>

                            <span>Passwort:</span>     <span>{entry.password}</span> <button onClick={() => copyAndClearClipboard(entry.password)}>🔗</button>

                        {/* adds https://www. to the start of the link*/}
                            <span>URL:</span>          <a href={entry.url.includes("www.") ? (entry.url.startsWith("http")?entry.url:("https://"+entry.url)) :("https://www."+entry.url)}
                                                          target="_blank" rel="noopener noreferrer"
                                                          style={{textDecoration: "underline", color: "inherit"}}>
                                                          {entry.url}</a>
                        <button onClick={() => copyAndClearClipboard(entry.url)}>🔗</button>

                            <span>Notiz:</span>        <span>{entry.note}</span> <button onClick={() => copyAndClearClipboard(entry.note)}>🔗</button>
                    </div>
                );
